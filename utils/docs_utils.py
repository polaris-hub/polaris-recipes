...
def visualize_split_chemspace(mols, train_test_dict):
    with dm.without_rdkit_log():
        X = dm.utils.parallelized(fn=dm.to_fp, inputs_list = mols, n_jobs=-1, progress=True)
    ...
    figs = visualize_chemspace(X=X, y=splits, labels=labels, umap_kwargs = {},
                               plot_kwargs={"hue_order": [ "nan", "train", "test",], "alpha":0.9})
